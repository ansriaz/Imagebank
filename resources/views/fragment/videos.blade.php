...
<form class="form-horizontal" id="detailform" role="form" method="GET" action="{{ url('/details/video') }}"> </form>
{{-- <form class="form-horizontal" id="downloadForm" role="form" method="GET" action="{{ url('/proceedToDownload') }}"> --}}
	...
	@foreach ($responseData as $obj)
		{{-- @if(count($obj['obj']) > 0) --}}
			<div class="panel panel-default">
				...
				<div class="playlist{{ $obj['id'] }}">
					{{-- @include('fragment.playlist',['obj'=>$obj,'title'=>$obj['title'], 'page' => $responseData['page']]) --}}
				</div>
			</div>
		{{-- @endif --}}
	@endforeach
	...
	@if( !isset($page) )
		<div class="pagination">
	    	{!! $responseData->appends(['dataset_title'=>$title,'type' => 'videos'])->render() !!}
	    </div>
    @endif
		...
		@foreach ($responseData as $obj)
			...
			$.ajax({
                ...
                success: function(response) {
                	...
                	{
                		$(".playlist{{ $obj['id'] }}").parent().hide();
                	} else {
                		$(".playlist{{ $obj['id'] }}").html(response);
                	}
                },
            ...
            $(document).on('click', ".pagination{{ $obj['id'] }} .pagination a", function (e) {
				...
				@endif
	            $.ajax({
		            url : '/getvideos' + '?page=' + page,
		            data: dataToSend,
		        }).done(function (data) {
		            $('.playlist{{ $obj['id'] }}').html(data);
		            location.hash = page;
		        }).fail(function () {
